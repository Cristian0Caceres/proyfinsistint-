...
# -------------------------------------------------------
# CONFIGURAR SALIDA UTF-8 (opcional)
# -------------------------------------------------------
try:
...
# -------------------------------------------------------
# RUTA COMPLETA AL ARCHIVO .NPY  <<< CAMBIA ESTA RUTA >>>
# -------------------------------------------------------
RUTA_NPY = r"C:\Users\ASUS F15\Documents\GitHub\proyfinsistint-\5\puntos_3d.npy"

print("="*70)
print("EJERCICIO 5: CENTROIDE DE PUNTOS AZULES EN 3D")
print("="*70)
...
print(f"Coordenadas del Centroide: ({centroide_x:.4f}, {centroide_y:.4f}, {centroide_z:.4f})")
...
ax1.scatter(df['X'], df['Y'], df['Z'], c='blue', alpha=0.5, s=30, 
            label='Puntos Azules', edgecolors='navy', linewidths=0.5)

ax1.scatter(centroide_x, centroide_y, centroide_z, c='red', s=500, 
...
ax1.set_ylabel('Y')
ax1.set_zlabel('Z')
ax1.set_title('Puntos 3D con Centroide')
ax1.legend()
ax1.grid(True, alpha=0.3)

ax1.text(centroide_x, centroide_y, centroide_z, 
         f'  C({centroide_x:.2f}, {centroide_y:.2f}, {centroide_z:.2f})',
         color='red', fontsize=10)

# Gráfico 2
ax2 = fig.add_subplot(122, projection='3d')
ax2.scatter(df['X'], df['Y'], df['Z'], c='blue', alpha=0.3, s=20, edgecolors='navy')
ax2.scatter(centroide_x, centroide_y, centroide_z, c='red', s=500,
            marker='*', edgecolors='yellow', linewidths=2)

# Dibujar ejes guía
ax2.plot([centroide_x, centroide_x], [centroide_y, centroide_y],
         [df['Z'].min(), df['Z'].max()], 'r--', alpha=0.5)

ax2.plot([centroide_x, centroide_x], [df['Y'].min(), df['Y'].max()],
         [centroide_z, centroide_z], 'r--', alpha=0.5)
...
ax2.set_ylabel('Y')
ax2.set_zlabel('Z')
ax2.set_title('Vista con Ejes del Centroide')
...
# XY
axes[0, 0].scatter(df['X'], df['Y'], c='blue', alpha=0.5)
axes[0, 0].scatter(centroide_x, centroide_y, c='red', s=200, marker='*')
axes[0, 0].set_title('Proyección XY')
axes[0, 0].axhline(centroide_y, color='r', linestyle='--')
axes[0, 0].axvline(centroide_x, color='r', linestyle='--')

# XZ
axes[0, 1].scatter(df['X'], df['Z'], c='blue', alpha=0.5)
axes[0, 1].scatter(centroide_x, centroide_z, c='red', s=200, marker='*')
axes[0, 1].set_title('Proyección XZ')
axes[0, 1].axhline(centroide_z, color='r', linestyle='--')
axes[0, 1].axvline(centroide_x, color='r', linestyle='--')

# YZ
axes[1, 0].scatter(df['Y'], df['Z'], c='blue', alpha=0.5)
axes[1, 0].scatter(centroide_y, centroide_z, c='red', s=200, marker='*')
axes[1, 0].set_title('Proyección YZ')
axes[1, 0].axhline(centroide_z, color='r', linestyle='--')
axes[1, 0].axvline(centroide_y, color='r', linestyle='--')
